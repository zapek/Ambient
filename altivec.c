 ...
 *
 * $Id: altivec.c,v 1.8 2021/01/31 21:48:53 piru Exp $
 */
...
ULONG use_altivec;
#endif
int libnix_altivec; /* for setjmp/longjmp */


ULONG altivec_init(void)
{
	NewGetSystemAttrs(&has_altivec, sizeof(has_altivec), SYSTEMINFOTYPE_PPC_ALTIVEC, TAG_DONE);
	libnix_altivec = has_altivec;

#if USE_ALTIVEC
	if (has_altivec && !_var(noaltivec))
	{
		/* Not all MorphOS versions have the proper
		 * software support for altivec, so lets make sure
		 * we only use it if the support is there.
		 */
