 ...
 *
 * $Id: am_gfx_blit.c,v 1.1 2026/01/25 17:39:37 kronos Exp $
 */
...
{
	size_t tags[] = { BLTBMA_MIXLEVEL, mixlevel, /*BLTBMA_DESTALPHAVALUE, DESTALPHAVALUE_USEDEST,*/ TAG_DONE };
	...
	BltBitMapAlpha(Src,offx,offy,Dst,DstBounds->MinX,DstBounds->MinY,FirstSizeX,FirstSizeY,(struct TagItem *)&tags); // blit the first piece of the tile

	if (SecondSizeX>0) // if SrcOffset was 0 or the dest rect was too narrow, we won't need a second column
		BltBitMapAlpha(Src,0,offy,Dst,SecondMinX,DstBounds->MinY,SecondSizeX,FirstSizeY,(struct TagItem *)&tags);

	if (SecondSizeY>0) // is a second row necessary?
	{
		BltBitMapAlpha(Src,offx,0,Dst,DstBounds->MinX,SecondMinY,FirstSizeX,SecondSizeY,(struct TagItem *)&tags);

		if (SecondSizeX>0)
			BltBitMapAlpha(Src,0,0,Dst,SecondMinX,SecondMinY,SecondSizeX,SecondSizeY,(struct TagItem *)&tags);
	}

	// this loop generates the first row of the tiles
	for (Pos = DstBounds->MinX+SrcSizeX,Size = min(SrcSizeX,DstBounds->MaxX-Pos+1);Pos<=DstBounds->MaxX;)
	{
		BltBitMapAlpha(Src,DstBounds->MinX,DstBounds->MinY,Dst,Pos,DstBounds->MinY,Size,min(SrcSizeY,RECTSIZEY(DstBounds)),(struct TagItem *)&tags);
		Pos += Size;
		Size = min(Size<<1,DstBounds->MaxX-Pos+1);
	}
	...
	for (Pos = DstBounds->MinY+SrcSizeY,Size = min(SrcSizeY,DstBounds->MaxY-Pos+1);Pos<=DstBounds->MaxY;)
	{
		BltBitMapAlpha(Src,DstBounds->MinX,DstBounds->MinY,Dst,DstBounds->MinX,Pos,RECTSIZEX(DstBounds),Size,(struct TagItem *)&tags);
		Pos += Size;
		Size = min(Size<<1,DstBounds->MaxY-Pos+1);
	}
}
...
void gfx_blit(APTR src, APTR dst, ...)
{
	...
	while(ti[i].ti_Tag = va_arg(args,ULONG)) 
	{
		ti[i].ti_Data = va_arg(args,ULONG);
	//	PDB(("%d %x\n",i,ti[i].ti_Tag));
		i++;
	}
	va_end(args);
	v_gfx_blit(src, dst, ti);
}
