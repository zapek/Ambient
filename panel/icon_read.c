...
#include "trash/mui_func.h"
#include "trash/pngicon_specs.h"
#include "gfx_bitmap.h"
#include "gfx_blit.h"
#include "trash/image.h"
#include "trash/classes.h"
#include "name.h"
...
ULONG pngio_sig_valid(CONST_STRPTR s)
{
 ...
 */
int read_chunk_call_back(png_structp png_ptr, png_unknown_chunkp chunk)
{
	if (chunk->name[0] == 'i'
		&& chunk->name[1] == 'c'
		&& chunk->name[2] == 'O'
...
ULONG png_icon_read(APTR fh, APTR obj, ULONG end, ULONG getimage)
{
...
ULONG byte_run1_decode(UBYTE *in, LONG srclen, ULONG depth, UBYTE *out, LONG dstlen)
{
	LONG n, bitshift = 0;
	UWORD shift = 0;
	UBYTE b;
...
void *z__alloc(void *p, int items, int size)
{
	return AllocVecTaskPooled(items * size);
}

void z__free(void *p, void *addr)
{
	if (addr)
...
#if 1
APTR gfx_mask_create_chunky_8(UBYTE * chunky, ULONG width, ULONG height, ULONG bgvalue)
{
	APTR mbm;
...
void gfx__alpha_set(APTR bm, ULONG x, ULONG y, ULONG width, ULONG height, UBYTE val)
{
				...
				else if (ch.id == ID_ARGB)
				{
					...
					if ((buf = icon_malloc(readsize)))
					{
						...
						if (file_read(fh, buf, readsize))
						{
							...
							stream.zalloc = (alloc_func)&z__alloc;
							stream.zfree = (free_func)&z__free;
...
APTR gfx__cmap_create(CONST ULONG *cmap32, ULONG colornum)
{
	struct ColorMap *cm;
