 ...
 *
 * $Id: gfx_scale.c,v 1.1 2015/03/02 13:50:55 geit Exp $
 */
...
#if USE_ALTIVEC
static ULONG gfx_scale_vec_average(struct gsi_info *gsii, CONST ULONG *s, ULONG x, ULONG y)
{
...
static void gfx_scale_scale2x(APTR sbm, ULONG width, ULONG height, APTR tbm)
{
	if (width && height)
	{
		...
		do
		{
			...
			do
			{
				register ULONG a, b, c, d, e, f, g, h, i;

				a = *(sp + SUB_MODULO + SUB_ONE);
				b = *(sp + SUB_MODULO);
				c = *(sp + SUB_MODULO + ADD_ONE);
				d = *(sp + SUB_ONE);
				e = *sp;
				f = *(sp + ADD_ONE);
				g = *(sp + ADD_MODULO + SUB_ONE);
				h = *(sp + ADD_MODULO);
				i = *(sp + ADD_MODULO + ADD_ONE);
 ...
 */
ULONG LIB_gfx_ScaleA(APTR sbm, APTR tbm, ULONG txs, ULONG tys, struct TagItem *tags)
{
	ULONG flags = 0;
	ULONG *rxs = NULL;
	ULONG *rys = NULL;
 ...
 */
void LIB_gfx_ScaleCalcAspectConstraints(ULONG sxs, ULONG sys, ULONG *txs, ULONG *tys)
{
	if (sxs > sys)
	{
		/* horizontal image */
 ...
 */
void LIB_gfx_ScaleCalcAspect(ULONG sxs, ULONG sys, ULONG *txs, ULONG *tys)
{
